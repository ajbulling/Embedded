...
typedef struct esos_menu_longmenu_item_t {
	char ac_line1[8];
	char ac_line2[8];
...
*/
typedef struct esos_menu_longmenu_t {
	uint8_t u8_numitems;
	uint8_t u8_choice;
...
*/
typedef struct esos_menu_staticmenu_t {
	uint8_t u8_numlines;
...
*/
typedef struct esos_menu_entry_item_t {
	char label[8];
	int16_t value;
	int16_t min;
	int16_t max;
...
esos_menu_entry_t Usage:
	entries[]: Contains the two visible entries on your screen.
			   ex: x = 147
				   y = 256
*/
typedef struct esos_menu_entry_t {
...
/*Notes: Since *pv_data is void, the value pv_data can point to any
  of our menu types.
*/
typedef struct __esos_menu_conf_t {
	enum {
		NONE,
		LONGMENU,
		STATICMENU,
